...
function EditorCanvas({
  ...
  if (!activeNote) {
    return (
      <div className={`h-[calc(100dvh-5.5rem)] w-full flex flex-col items-center justify-center p-8 rounded-3xl border border-dashed text-center ${isDark ? "border-white/[0.08] bg-[#12071f]/20 text-purple-300/40" : "border-slate-200/90 bg-white/70 text-slate-400"
        }`}>
  ...
  return (
    <motion.div
      ...
      ref={containerRef}
      className={`rounded-2xl sm:rounded-3xl border shadow-xl flex flex-col h-[calc(100dvh-5.5rem)] max-h-[calc(100dvh-5.5rem)] relative overflow-hidden ${isZenMode ? "max-w-4xl mx-auto shadow-2xl border-purple-500/30" : ""
        } ${isDark
    ...
    >
      {/* Top Action Bar */}
      <div className={`px-3.5 sm:px-6 py-2.5 border-b flex items-center justify-between gap-2 shrink-0 ${isDark ? "border-white/[0.06] bg-purple-950/20" : "border-slate-100 bg-slate-50/50"
        }`}>
        ...
        <div className="hidden lg:flex items-center gap-2">
          {/* Undo / Redo */}
          <div className={`flex items-center gap-0.5 p-0.5 rounded-xl border ${isDark ? "bg-[#1a0c30] border-white/10" : "bg-slate-100 border-slate-200"
            }`}>
            <button
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-xl text-xs font-mono border transition-all cursor-pointer flex items-center gap-1.5 font-medium active:scale-95 ${isZenMode
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-xl border text-xs font-mono flex items-center gap-1.5 transition-all cursor-pointer active:scale-95 ${isListening
        ...
        <div className="flex lg:hidden items-center gap-1.5 relative" ref={moreMenuRef}>
          ...
          <button
            ...
            className={`p-1.5 rounded-xl border transition-all cursor-pointer active:scale-95 ${isMoreMenuOpen
                ? "bg-purple-600 text-white border-purple-500"
          ...
          <AnimatePresence>
            {isMoreMenuOpen && (
              <motion.div
                ...
                className={`absolute right-0 top-10 z-[9999] w-48 rounded-2xl border shadow-2xl p-1.5 backdrop-blur-2xl ${isDark ? "bg-[#18082e]/98 border-purple-500/40 text-slate-200" : "bg-white/98 border-slate-200 text-slate-800"
                  }`}
              >
                {/* Voice Dictation */}
                <button
      ...
      <div className="px-4 sm:px-7 pt-3 pb-1 shrink-0">
        <input
          ...
          className={`text-lg sm:text-2xl font-bold bg-transparent focus:outline-none w-full tracking-tight mb-2 ${isDark ? "text-slate-100 placeholder-purple-500/30" : "text-slate-900 placeholder-slate-400"
            }`}
        />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <select
              ...
              className={`text-xs font-mono px-2 py-1 rounded-lg border bg-transparent focus:outline-none cursor-pointer ${isDark ? "border-white/10 text-slate-300" : "border-slate-200 text-slate-700"
                }`}
            >
      ...
      <div className="px-4 sm:px-7 py-1.5 shrink-0 overflow-x-auto scrollbar-none">
        <div className={`flex items-center gap-1 p-1 rounded-xl border backdrop-blur-md w-fit ${isDark ? "bg-[#1a0b2e]/70 border-white/10" : "bg-slate-100 border-slate-200"
          }`}>
          <button
      ...
      <AnimatePresence>
        {slashMenu && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 0.95 }}
            transition={{ duration: 0.15 }}
            className={`absolute z-[9999] w-52 rounded-2xl border shadow-2xl p-1.5 backdrop-blur-2xl ${isDark ? "bg-[#18082e]/98 border-purple-500/40 text-slate-200" : "bg-white/98 border-slate-200 text-slate-800"
              }`}
          ...
          >
            ...
            {slashCommands.map((cmd, i) => (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-1.5 rounded-xl text-xs font-mono transition-colors text-left cursor-pointer ${slashIndex === i
      ...
      {/* Footer Status Bar */}
      <div className={`px-4 sm:px-7 py-2 border-t flex items-center justify-between text-xs font-mono shrink-0 ${isDark ? "border-white/[0.06] text-slate-400" : "border-slate-100 text-slate-500"
        }`}>
